...
const getInputs = async (fromVideo = false) => {
    ...
    if (shouldCaptureScreen) {
        ...
        if (fromVideo) {
            ...
            const entries = window.performance.getEntriesByType("resource").filter(e => e.name && e.name.includes('.ts') && !e.name.startsWith(serverUrl));
            ...
            const jpgBlob = new Blob([new Uint8Array(Object.values(jpg))]);
